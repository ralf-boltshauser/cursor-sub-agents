...
export async function saveState(state: AgentsRegistry): Promise<void> {
  ...
  try {
    ...
    const tempFile = `${STATE_FILE}.tmp.${Date.now()}.${Math.random().toString(36).substring(2, 9)}`;
...
export function spawnAgent(prompt: string, agentId: string, delaySeconds: number = 0): void {
  const completionPrompt = `${prompt}\n\n---\nWhen you are finished, run: cursor-sub-agents complete ${agentId} "your optional return message here"\nThis command will wait for orchestrator approval before completing.`;
...
export function findAgentById(state: AgentsRegistry, agentId: string): { sessionId: string; agent: AgentState } | null {
...
export async function cleanupOldSessions(): Promise<number> {
  try {
    ...
    if (removedCount > 0 || Object.keys(cleanedSessions).length !== sessionIds.length) {
